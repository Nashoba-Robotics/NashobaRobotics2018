...
public class SwitchLeftToBlockProfilingCommand extends CommandGroup {
	...
	public SwitchLeftToBlockProfilingCommand() {
		...
		addSequential(new EnableMotionProfile((FieldMeasurements.SWITCH_EDGE_TO_FIELD_EDGE_Y.sub(FieldMeasurements.FAR_START_POS_TO_FIELD_EDGE_Y).sub(FieldMeasurements.ROBOT_LENGTH.mul(0.5))).negate(), 
		...
		addSequential(new EnableMotionProfile(FieldMeasurements.SWITCH_TO_PLATFORM_ZONE_X.negate(), Distance.ZERO, 
					Drive.PROFILE_DRIVE_PERCENT, Drive.ACCEL_PERCENT));
		
		addSequential(new EnableLimelightCommand(true));
		
		addSequential(new TurnPIDCommand(Drive.getInstance(), (new Angle(90, Angle.Unit.DEGREE).sub(FieldMeasurements.PLATFORM_ZONE_TO_CUBE)).negate(), 
