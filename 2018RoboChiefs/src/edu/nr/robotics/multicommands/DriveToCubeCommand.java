...
public class DriveToCubeCommand extends NRCommand {
	...
	protected void onExecute() {
		...
		if (stoppedTracking) {
			...
			if (LimelightNetworkTable.getInstance().getHorizOffset().abs().lessThan(new Angle(180, Angle.Unit.DEGREE))) {
				headingAdjustment = ((-Math.cos(LimelightNetworkTable.getInstance().getHorizOffset().get(Angle.Unit.RADIAN) / 2) * (1 - Drive.MIN_PROFILE_TURN_PERCENT)) + 1 + Drive.MIN_PROFILE_TURN_PERCENT) * -LimelightNetworkTable.getInstance().getHorizOffset().signum();
			} else {
				headingAdjustment = -LimelightNetworkTable.getInstance().getHorizOffset().signum();
			}
			
			if(Math.abs(headingAdjustment) > 1) {
				headingAdjustment = Math.signum(headingAdjustment);
			}
