...
public class IntakeElevator extends NRSubsystem implements PIDSource, PIDOutput {
	 ...
	 */
	public static final Acceleration MAX_ACCEL_INTAKE_ELEVATOR_UP = new Acceleration(12, Distance.Unit.FOOT,
			Time.Unit.SECOND, Time.Unit.SECOND);
	public static final Acceleration MAX_ACCEL_INTAKE_ELEVATOR_DOWN = new Acceleration(12, Distance.Unit.FOOT,
			Time.Unit.SECOND, Time.Unit.SECOND);
	...
	/**
	 * The voltage ramp rate of the intake elevator. Voltage ramp rate is time it
	 * takes to go from 0V to 12V
	 */
	...
	/**
	 * The distance from the end of the intake elevator profile at which the
	 * stopping algorithm starts
	 */
	public static final Distance PROFILE_END_POS_THRESHOLD_INTAKE_ELEVATOR = new Distance(1, Distance.Unit.INCH);

	public static final Speed PROFILE_STOP_SPEED_THRESHOLD = new Speed(0.1, Distance.Unit.INCH, Time.Unit.SECOND); // TODO:
																													// Decide
																													// on
																													// this
	...
	public static final double kV_UP = 1 / MAX_SPEED_INTAKE_ELEVATOR_UP
	...
	public static final double kV_DOWN = 1 / MAX_SPEED_INTAKE_ELEVATOR_DOWN
			.get(Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV, Time.Unit.HUNDRED_MILLISECOND);
	...
	public static final Distance PORTAL_HEIGHT = INTAKE_HEIGHT.add(new Distance(3, Distance.Unit.INCH)); // TODO: Find
																											// PORTAL_HEIGHT
	...
	private IntakeElevator() {
		...
		if (EnabledSubsystems.INTAKE_ELEVATOR_ENABLED) {
			...
			intakeElevTalon.configClosedloopRamp(VOLTAGE_RAMP_RATE_INTAKE_ELEVATOR.get(Time.Unit.SECOND),
					DEFAULT_TIMEOUT);
			intakeElevTalon.configOpenloopRamp(VOLTAGE_RAMP_RATE_INTAKE_ELEVATOR.get(Time.Unit.SECOND),
					DEFAULT_TIMEOUT);

			intakeElevTalon.configMotionCruiseVelocity(
					(int) MAX_SPEED_INTAKE_ELEVATOR_UP.mul(PROFILE_VEL_PERCENT_INTAKE_ELEVATOR)
							.get(Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV, Time.Unit.HUNDRED_MILLISECOND),
					DEFAULT_TIMEOUT);
			intakeElevTalon.configMotionAcceleration((int) MAX_ACCEL_INTAKE_ELEVATOR_UP
					.mul(PROFILE_ACCEL_PERCENT_INTAKE_ELEVATOR).get(Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV,
							Time.Unit.HUNDRED_MILLISECOND, Time.Unit.HUNDRED_MILLISECOND),
					DEFAULT_TIMEOUT);

			intakeElevTalon.configReverseLimitSwitchSource(LimitSwitchSource.FeedbackConnector,
					LimitSwitchNormal.NormallyOpen, DEFAULT_TIMEOUT);
	...
	public Distance getPosition() {
		if (intakeElevTalon != null) {
			return new Distance(intakeElevTalon.getSelectedSensorPosition(PID_TYPE),
					Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV);
		}
	...
	public Speed getVelocity() {
		if (intakeElevTalon != null) {
			return new Speed(intakeElevTalon.getSelectedSensorVelocity(PID_TYPE),
	...
	public void setPosition(Distance position) {
		if (intakeElevTalon != null) {
			...
			if (position.sub(getPosition()).greaterThan(Distance.ZERO)) {
				...
				intakeElevTalon.configMotionCruiseVelocity(
						(int) MAX_SPEED_INTAKE_ELEVATOR_DOWN.mul(PROFILE_VEL_PERCENT_INTAKE_ELEVATOR)
								.get(Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV, Time.Unit.HUNDRED_MILLISECOND),
						DEFAULT_TIMEOUT);
				intakeElevTalon.configMotionAcceleration((int) MAX_ACCEL_INTAKE_ELEVATOR_DOWN
						.mul(PROFILE_ACCEL_PERCENT_INTAKE_ELEVATOR).get(Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV,
								Time.Unit.HUNDRED_MILLISECOND, Time.Unit.HUNDRED_MILLISECOND),
						DEFAULT_TIMEOUT);
			}
		...
		if (dist.greaterThan(Distance.ZERO)) {
			...
			basicProfiler.setTrajectory(
					new OneDimensionalTrajectoryRamped(dist.get(Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV),
							MAX_SPEED_INTAKE_ELEVATOR_UP.mul(PROFILE_VEL_PERCENT_INTAKE_ELEVATOR).get(
									Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV, Time.Unit.HUNDRED_MILLISECOND),
			...
			basicProfiler.setTrajectory(
					new OneDimensionalTrajectoryRamped(dist.get(Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV),
							MAX_SPEED_INTAKE_ELEVATOR_DOWN.mul(PROFILE_VEL_PERCENT_INTAKE_ELEVATOR).get(
									Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV, Time.Unit.HUNDRED_MILLISECOND),
		...
		if (intakeElevTalon != null) {
			...
			if (speed.greaterThan(Speed.ZERO)) {
				...
				intakeElevTalon.config_kF(VEL_UP_SLOT,
						((VOLTAGE_PERCENT_VELOCITY_SLOPE_INTAKE_ELEVATOR_UP
								* velSetpoint.abs().get(Distance.Unit.FOOT, Time.Unit.SECOND)
				...
				if (EnabledSubsystems.INTAKE_ELEVATOR_DUMB_ENABLED) {
					...
					intakeElevTalon.set(ControlMode.Velocity, velSetpoint
							.get(Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV, Time.Unit.HUNDRED_MILLISECOND));
				}
			} else {
				intakeElevTalon.selectProfileSlot(VEL_DOWN_SLOT, DEFAULT_TIMEOUT);
				intakeElevTalon.config_kF(VEL_DOWN_SLOT,
						((VOLTAGE_PERCENT_VELOCITY_SLOPE_INTAKE_ELEVATOR_DOWN
								* velSetpoint.abs().get(Distance.Unit.FOOT, Time.Unit.SECOND)
	...
	public void smartDashboardInfo() {
		if (EnabledSubsystems.INTAKE_ELEVATOR_SMARTDASHBOARD_BASIC_ENABLED) {
			...
			SmartDashboard.putNumberArray("Intake Elevator Velocity vs. Set Velocity: ",
					new double[] { getVelocity().get(Distance.Unit.FOOT, Time.Unit.SECOND),
							velSetpoint.get(Distance.Unit.FOOT, Time.Unit.SECOND) });
			SmartDashboard.putNumberArray("Intake Elevator Position vs. Set Position: ",
					new double[] { getPosition().get(Distance.Unit.INCH), posSetpoint.get(Distance.Unit.INCH) });
			/*
			 ...
			 * getPosition().get(Distance.Unit.INCH) + " : " +
			 * posSetpoint.get(Distance.Unit.INCH));
			 */
		}
		if (EnabledSubsystems.INTAKE_ELEVATOR_SMARTDASHBOARD_DEBUG_ENABLED) {
			...
			F_POS_INTAKE_ELEVATOR_DOWN = SmartDashboard.getNumber("F Pos Intake Elevator Down: ",
					F_POS_INTAKE_ELEVATOR_DOWN);
			P_POS_INTAKE_ELEVATOR_DOWN = SmartDashboard.getNumber("P Pos Intake Elevator Down: ",
					P_POS_INTAKE_ELEVATOR_DOWN);
			I_POS_INTAKE_ELEVATOR_DOWN = SmartDashboard.getNumber("I Pos Intake Elevator Down: ",
					I_POS_INTAKE_ELEVATOR_DOWN);
			D_POS_INTAKE_ELEVATOR_DOWN = SmartDashboard.getNumber("D Pos Intake Elevator Down: ",
					D_POS_INTAKE_ELEVATOR_DOWN);
			P_VEL_INTAKE_ELEVATOR_DOWN = SmartDashboard.getNumber("P Vel Intake Elevator Down: ",
					P_VEL_INTAKE_ELEVATOR_DOWN);
			I_VEL_INTAKE_ELEVATOR_DOWN = SmartDashboard.getNumber("I Vel Intake Elevator Down: ",
					I_VEL_INTAKE_ELEVATOR_DOWN);
			D_VEL_INTAKE_ELEVATOR_DOWN = SmartDashboard.getNumber("D Vel Intake Elevator Down: ",
					D_VEL_INTAKE_ELEVATOR_DOWN);
			...
			SmartDashboard.putNumber("Intake Elevator Encoder Ticks: ",
					intakeElevTalon.getSelectedSensorPosition(PID_TYPE));
		}
	}
	...
	public void periodic() {
		if (EnabledSubsystems.INTAKE_ELEVATOR_ENABLED) {
			if (intakeElevTalon.getSensorCollection().isRevLimitSwitchClosed()) {
				intakeElevTalon.getSensorCollection().setQuadraturePosition(
						(int) BOTTOM_HEIGHT.get(Distance.Unit.MAGNETIC_ENCODER_TICK_INTAKE_ELEV), DEFAULT_TIMEOUT);
			}
		}
